...
function enhanced = init_plot_stream (plot_stream, h, term, file)
  ...
  if (nargin == 4)
    ...
    if (! isempty (term))
      if (enhanced)
        term = split(term, " ", 2); # 2008-04-13 to append size option for PDF device
        # size option must be end of the line.
	fprintf (plot_stream, "set terminal %s enhanced %s;\n", deblank(term(1,:)), term(2,:));
      else
