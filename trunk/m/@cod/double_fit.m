...
function [cod_FB, cod_FT] = ...
              double_fit(cod_FB, cod_FT, variableKickers, varargin);
  #  cod_FB = codRec_FB_correct;
  #  cod_FT = codRec_correct_FT;
  #  variableKickers = {"QD2"};

  use_kick_factors = false;
  use_weight = false;
  for n = 1:length(varargin)
    if (strcmp(varargin{n}, "use_kick_factors"))
      if (isfield(cod_FB, "kick_factors"))
  ...
  codMatStruct_FB = build_matrix(cod_FB);
  codMatStruct_FT = build_matrix(cod_FT);
  steerer_names = cod_FB.kickers;
    ...
    for n = 1:length(kickers)
      for m = 1:length(steerer_names)
        if (strcmp(kickers{n}.name, steerer_names{m}))
          kick_factors_FB(end+1) = cod_FB.kick_factors(m);
          kick_factors_FT(end+1) = cod_FT.kick_factors(m);
        endif
      endfor
    endfor
  ...
  if (use_weight)
    dy = [];
    dy = [dy; extract_dy(cod_FB, codMatStruct_FB)];
    dy = [dy; extract_dy(cod_FT, codMatStruct_FT)];
    dy = 1./dy;
  ...
  if (use_kick_factors)
    kick_angles_sorted_FB = kick_angles_sorted_FB./cod_FB.kickFactors;
    kick_angles_sorted_FT = kick_angles_sorted_FT./cod_FT.kickFactors;
  endif
  
  cod_FB.kick_angles = kick_angles_sorted_FB;
  cod_FT.kick_angles = kick_angles_sorted_FT;
  
#  cod_FB.correctCOD = calcCODWithPerror(cod_FB);
#  cod_FT.correctCOD = calcCODWithPerror(cod_FT);
#  xyplot(cod_FB.targetCOD, "-@"\
#    , cod_FB.correctCOD, "-"\
#    , cod_FT.targetCOD, "-@"\
#    , cod_FT.correctCOD, "-")
endfunction
